...
def scrape_all(start_url, jurusan, seen_urls, max_pages=None):
    all_data = []
    ...
    while current_url:
        ...
        items, next_url = scrape_page(current_url, jurusan)
...
def save_to_xlsx(data, filename):
    ...
    try:
        if file_exists:
            ...
            headers = ['Judul', 'NIM', 'Tahun', 'Penulis', 'Jurusan', 'File']
        ...
        for idx, item in enumerate(data, start_row):
            ...
            ws.cell(row=idx, column=5, value=item['jurusan'])
            ws.cell(row=idx, column=6, value=item['file'])
...
def main():
    ...
    for idx, (url, jurusan) in enumerate(URLS_TO_SCRAPE, 1):
        print(f"\n{'='*60}")
        print(f"URL {idx}/{len(URLS_TO_SCRAPE)} - Jurusan: {jurusan}")
        print(f"{'='*60}")
        
        data = scrape_all(url, jurusan, seen_urls)
        
        if data:
            save_to_xlsx(data, filename)
            total_data += len(data)
            print(f"Berhasil: {len(data)} data baru dari URL {idx} ({jurusan})")
        else:
            print(f"Tidak ada data baru dari URL {idx} ({jurusan})")
